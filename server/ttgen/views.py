...
class Timetable(APIView):
    ...
    def get(self, request, format=None):
        ...
        sections_serializer = SectionSerializer( Section.objects.all(), many=True)
        times_serializer = MeetingTimeSerializer( MeetingTime.objects.all(), many=True)
        toReturn = [{
            "section": x.section,
            "section_id": x.section_id,
            "course": CourseSerializer(x.course, many=False).data,
            "room" : RoomSerializer(x.room, many=False).data,
            "instructor" : InstructorSerializer(x.instructor, many=False).data,
            "meeting_time": MeetingTimeSerializer(x.meeting_time, many=False).data
        } for x in schedule]

        return Response( {'schedule': toReturn, 'sections':sections_serializer.data,
                                                        'times': times_serializer.data})
...
class TimeSlot(APIView):
    
    
    def get(self, request, format=None):
        return Response( {'time_slots': [i for (i,_) in time_slots], 'days_of_week': [i for (i,_) in DAYS_OF_WEEK]})
