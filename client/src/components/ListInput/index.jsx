...
function ListInput({ items, indices, setIndices }) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedPersons, setSelectedPersons] = useState([]);

  function isSelected(value) {
    return selectedPersons.find((el) => el.name === value) ? true : false;
  }

  useEffect(() => {
    setIndices(selectedPersons);
  }, [selectedPersons]);
            ...
            <div className="relative">
              ...
              <div className="pt-1 text-sm">
                {selectedPersons.length > 0 && (
                  <>
                    Selected : {selectedPersons.map((el) => el.name).join(", ")}
                  </>
                )}
              </div>
            </div>
